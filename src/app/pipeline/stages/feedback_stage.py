...
class FeedbackStage(ProcessingStage):
    ...
    async def process(self, scraping_context: ScrapingContext, nextStep=None):
        ...
        if current_time - self.last_sent_time >= self.interval:
            ...
            # Logging and printing feedback sending event
            logger.info(f"Sending feedback with top {self.top_k} named entities: {top_entities}")
            print(f"[INFO] Sending feedback with top {self.top_k} named entities: {top_entities}")
